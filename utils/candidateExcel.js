...
const COLUMN_ALIASES = {
  candidateId: ['Candidate ID', 'candidateId', '_id', 'ID'],
  name: ['Name', 'name'],
  email: ['Email', 'email'],
...
const rowToPayload = (row = {}) => {
  const candidateId = String(pickField(row, COLUMN_ALIASES.candidateId)).trim();
  return {
    candidateId: mongoose.Types.ObjectId.isValid(candidateId) ? candidateId : '',
...
const candidateToExportRow = (c) => ({
  'Candidate ID': c._id ? String(c._id) : '',
  Name: c.name || '',
  Email: c.email || '',
  Phone: c.phone || '',
...
const duplicateKey = (payload) => {
  if (payload.candidateId) return `id:${payload.candidateId}`;
